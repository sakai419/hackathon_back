...
func (s *Service) SearchTweetsByHashtag(ctx context.Context, params *model.SearchTweetsByHashtagParams) ([]*model.TweetNode, error) {
	// Validate input
	if err := params.Validate(); err != nil {
		return nil, apperrors.NewValidateAppError(err)
	}
	...
	switch params.SortType {
		case model.SortTypeLatest, "":
			temp, err := s.repo.SearchTweetsByHashtagOrderByCreatedAt(ctx, &model.SearchTweetsByHashtagOrderByCreatedAtParams{
				ClientAccountID: params.ClientAccountID,
			...
			tweets = temp
		case model.SortTypePopular:
			temp, err := s.repo.SearchTweetsByHashtagOrderByEngagementScore(ctx, &model.SearchTweetsByHashtagOrderByEngagementScoreParams{
				ClientAccountID: params.ClientAccountID,
				Hashtag: params.Hashtag,
				Offset: params.Offset,
				Limit: params.Limit,
